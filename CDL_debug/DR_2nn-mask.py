...
class DR_CML(nn.Module):  # naive upper bound
    ...
    def forward(self, x_samples, y_samples): # x_samples = s_t, a ; y_samples = s_{t+1}
        ...
        w = 1 / propensity_score
        # w0 = torch.mean(w[x_samples[:,-1] == -5])
        # w1 = torch.mean(w[x_samples[:,-1] != -5])
        ...
        for k in range(y_samples.shape[1]):
            ...
            dr = (cmi_dim + w*((positive.unsqueeze(-1)- negative) -cmi_dim)).mean()
...
Dim = 5
batch_size = 10
dataset = CMINE.create_dataset_DGP(GenModel="", Params="", Dim=5, N=batch_size)
