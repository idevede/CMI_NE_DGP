...
class InferenceCMI(Inference):
    ...
    def update_mask(self, obs, actions, next_obses):
        ...
        # CMI = CMI.mean(dim=0) 
        # ----------------- New -----------------
        CMI_mean = CMI.mean(dim=0)    # (feature_dim, feature_dim)
        unique_elements = torch.unique(actions)
        actions = actions.reshape(-1)
        CMI_copy = torch.zeros_like(CMI)
        for i in unique_elements:
            CMI_copy += 1/len(unique_elements)*(CMI_mean - (CMI[torch.nonzero(actions == i)]-CMI_mean).mean(dim=0))

        CMI = CMI_copy.mean(dim=0) 
        #----------------- New -----------------


        
        #masked_pred_losses[torch.nonzero(actions.reshape(-1) == 1)]
